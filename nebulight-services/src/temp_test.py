...
def get_trend_change(symbol: str, months: int):
    ...
    # 3 months before end_date
    start_date = end_date - relativedelta(months=months)

    # Fetch historical data for the symbol
    data = yf.download(symbol, start=start_date.strftime(
        '%Y-%m-%d'), end=end_date.strftime('%Y-%m-%d'))

    if data.empty:
        raise ValueError(f"No data found for {symbol} between {
    ...
    data['Signal'][data['EMA_12'] < data['EMA_26']] = - \
        1  # Downward trend signal
    ...
    trend_changes = [
        {
            # Corrected part
            "date": pd.to_datetime(signal.Index).strftime('%Y-%m-%d'),
            "price": signal.Close,
...
def spx_trend():
    ...
    top_n_tickers = sorted(
        market_caps, key=market_caps.get, reverse=True)[:100]
    ...
    for ticker in top_n_tickers:
        try:
            ...
            if (vol > 2.2):
                ...
                results[ticker] = {
                    "signals": trend_change_detected, "vol": vol}
